...
validate_input <- function(data, start, end, events, groups, linewidth, title, showLabels, lineInterval, background_lines){
  ...
  if (class(try(as.POSIXct(data[, start]), silent = T))[1] == "try-error") stop(paste("date format error: please make sure columns", start, "and", end, "can be converted to POSIXct type"))
  ...
  if (!class(background_lines) == "numeric") stop("background_lines must be numeric.")
