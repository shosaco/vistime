...
test_that("color columns not existing", {
  result <- vistime:::set_colors(data.frame(event = 1:4, start = c(Sys.Date(), Sys.Date() + 10),
                                            end = c(Sys.Date(), Sys.Date() + 10)), eventcolor_column = "NOTEXISTING", fontcolor_column = "NOTEXISTING")
